...
def process(input_fasta_path, k, epochs, activation, layers_sizes, train_perc, config_path = backend_path+'/config.ini'):
    ...
    data = reader.process(config, input_fasta_path)

    print('loaded input file')
    vectorizer.process(config, data, k)
    print('vectorizing done')
    gc_counter.process(config, data)
    print('gc count done')
...
def process_ae(input_fasta_path, k, epochs, activation, layers_sizes, train_perc, data, config_path = backend_path+'/config.ini'):
    config = parse_config(config_path)
